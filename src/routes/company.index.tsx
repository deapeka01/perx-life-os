...
function SideRail() {
  ...
  const openTopup = () => {
    setInitial({
      kind: "wallet_topup",
      amountALL: 0,
      description: `Wallet top-up for ${currentCompany.name}`,
      payerLabel: currentCompany.name,
    });
    setTopupOpen(true);
  };
  return (
    <div className="space-y-4">
      <div className="rounded-3xl border border-white/40 bg-white/70 p-5 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.35)] backdrop-blur-xl">
        ...
        <button
          onClick={openTopup}
          className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-2xl bg-coral px-4 py-2.5 font-display text-sm font-extrabold text-white shadow-coral transition hover:bg-coral/90"
        >
          <Banknote className="size-4" /> Fund wallet by bank transfer
        </button>
      </div>
      <PayByBankDialog open={topupOpen} onOpenChange={setTopupOpen} initial={initial} />
