...
import { Wallet, Target, Trophy, LogOut } from "lucide-react";
  ...
  return (
    <div className="px-5 pb-10 pt-8 sm:px-8 md:px-10 md:pt-12">
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-3">
        ...
        <StatTile label="Active goals" value={currentEmployee.goals.length} icon={<Target className="size-4" />} delta="2 on track" />
      ...
      <section className="mt-10 grid gap-6 lg:grid-cols-2">
        <div className="rounded-3xl border border-border bg-card p-6 shadow-soft">
          ...
          <div className="mt-5 space-y-5">
            {currentEmployee.goals.map((g) => (
              <div key={g.id}>
                <div className="mb-2 flex justify-between text-sm">
                  <span className="font-bold text-navy">{g.label}</span>
            ...
            {[
              ...
              <li key={i} className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-navy/45">{a.w}</p>
                </div>
              </li>
            ))}
          </ul>
